...
$cloudInit = @(
    ...
    '  # Update-Watcher: Host-seitiger Service der data/.update-trigger beobachtet',
    '  # und bei Auftauchen git pull + docker compose up --build ausfuehrt.',
    '  - |',
    "    cat > /opt/exo-gateway/update-watcher.sh << 'WATCHEOF'",
    '    #!/bin/bash',
    '    set -euo pipefail',
    ...
    '        rm -f "$TRIGGER"',
    '        write_status "{\"state\":\"running\",\"log\":\"git pull...\",\"version_before\":\"$VER_BEFORE\"}"',
    '        LOG=$(cd "$REPO" && git pull 2>&1) && \\',
    '          LOG2=$(cd "$REPO" && docker compose up -d --build 2>&1) && \\',
    '          VER_AFTER=$(cat "$REPO/VERSION" 2>/dev/null | tr -d "[:space:]" || echo "?") && \\',
    '          write_status "{\"state\":\"success\",\"finished\":\"$(date -u +%Y-%m-%dT%H:%M:%SZ)\",\"version_before\":\"$VER_BEFORE\",\"version_after\":\"$VER_AFTER\",\"log\":$(printf "%s\\n%s" "$LOG" "$LOG2" | python3 -c '\''import json,sys; print(json.dumps(sys.stdin.read()))'\'' )}" || \\',
